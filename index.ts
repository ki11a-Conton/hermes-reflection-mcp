...
import {
  ...
  bulkRetrieveHeuristics,
  type HeuristicWithScore,
  searchReflections,
  listReflections,
  diffReflections,
  type ReflectionDiffSummary,
  ...
  type ImportMode,
  generateProjectExperienceMarkdown,
  safeMarkdownFilename,
  getDomainSummary,
  type DomainSummary,
} from "./storage.js";
import type { ReflectionFrame, AffordanceGap, ReflectionStore } from "./types.js";
...
function stripMarkdown(text: string): string {
  return text
    ...
    .replace(/`([^`]+)`/g, "$1")
    .replace(/\|[^\n]+\|/g, (row) =>
      row.split("|").filter(Boolean).map((cell) => cell.trim()).join("; ")
    )
    .replace(/^[-*+]\s+/gm, "")
    .replace(/^\d+\.\s+/gm, "")
    .replace(/\n{3,}/g, "\n\n")
...
function prepareReflectionSave(input: ReflectInput): { save: BatchReflectionSaveInput; extractedCount: number; skippedUnsafeCount: number; gapLine: string; heuristicLine: string } {
...
Session [${input.session_id.slice(0, 8)}]: ${session.reflection_count} reflection(s) this session.${reflectionLimitWarning}`);
      ...
      case "bulk_reflect": {
        ...
        const prepared: BatchReflectionSaveInput[] = [];
        ...
        const lines = results.map((result, index) =>
          `${index + 1}. [${result.id}] ${result.outcome.toUpperCase()} - ${result.task_goal} (heuristics_extracted:${result.heuristics_extracted})`
        );
        const reflectionLimitWarning = nearSoftLimit
          ? `\n\n[WARN] Reflection store has ${reflectionCount} entries (soft limit: ${REFLECTION_SOFT_LIMIT}). Consider exporting and archiving old data with export_data(output_path=...).`
          : "";
      ...
      case "resolve_affordance_gap": {
        ...
        if (!gap) return err(`No affordance gap found with id: ${input.id}`);
      ...
      case "bulk_retrieve_heuristics": {
        ...
        const allResults = await bulkRetrieveHeuristics(
      ...
      case "contradict_heuristic": {
        ...
        if (!heuristic) return err(`No heuristic found with id: ${input.id}`);
      ...
      case "pin_heuristic": {
        ...
        if (!heuristic) return err(`No active heuristic found with id: ${input.id}`);
      ...
      case "update_heuristic": {
        ...
        if (!updated) return err(`No heuristic found with id: ${input.id}`);
      ...
      case "merge_heuristics": {
        ...
        if (!result) return err(`Target heuristic not found or already archived: ${input.target_id}`);
      ...
      case "get_heuristic_history": {
        ...
        if (!history) return err(`No heuristic found with id: ${input.id}`);
      ...
      case "diff_reflections": {
        ...
        if (!result) return err(`One or both reflections were not found: ${input.id_a}, ${input.id_b}`);
...
${lessonList || "  (none yet)"}${summary.metadata ? `\nStore metadata:\n  Created: ${summary.metadata.created_at.slice(0, 10)}\n  Write count: ${summary.metadata.write_count}` : ""}`);
      ...
      case "get_session_summary": {
        ...
        if (!summary) return err(`No session found: ${input.session_id}`);
...
${qLines}`);
      ...
      case "get_reflection": {
        ...
        if (!reflection) return err(`No reflection found with id: ${input.id}`);
...
Task state:${blockers}${safePaths}${deadEnds}${hypotheses}${tags}${lessons}${worldUpdates}${toolInsights}${openQuestions}${contextForget}`);
      ...
      case "update_reflection": {
        ...
        if (!result) return err(`Reflection not found: ${input.id}`);
...
Pass output_path to write the JSON to a file, or use a smaller collection export. You can also inspect the store file directly at ${STORE_DIR}.`);
      ...
      case "import_data": {
        ...
        const counts = await importData(parsed, input.mode as ImportMode);
...
function formatReflectionDiff(diff: ReflectionDiffSummary): string {
  ...
  const worldLines = diff.world_model_polarity_changes.length > 0
    ? diff.world_model_polarity_changes.map((item) =>
        `  - ${item.fact_a} [${item.polarity_a}] -> ${item.fact_b} [${item.polarity_b}]`
      ).join("\n")
    : "  (none)";
...
function formatStoreHealth(report: StoreHealthReport): string {
...
function formatHeuristicStats(stats: HeuristicStats): string {
  const domains = Object.entries(stats.domain_breakdown)
    .sort(([, a], [, b]) => b.count - a.count)
    .map(([domain, entry]) =>
      `  ${domain}: ${entry.count} active | avg confidence ${(entry.avg_confidence * 100).toFixed(1)}% | avg retrieved x${entry.avg_retrieval_count}`
    )
    .join("\n");
  const topRetrieval = stats.top_by_retrieval.length > 0
    ? stats.top_by_retrieval.map((item, index) =>
        `  ${index + 1}. [${item.domain}] x${item.retrieval_count} id:${item.id} ${item.heuristic}`
      ).join("\n")
    : "  (none)";
  const topReinforcement = stats.top_by_reinforcement.length > 0
    ? stats.top_by_reinforcement.map((item, index) =>
        `  ${index + 1}. [${item.domain}] x${item.reinforcement_count} id:${item.id} ${item.heuristic}`
      ).join("\n")
    : "  (none)";
...
function formatTimelineBucket(bucket: TimelineBucket): string {
