import type { HookEvent } from "../../hook_inbox.js";
import type { CanonicalLifecycleEvent } from "../../lifecycle/events.js";
...
export interface CodexEventMappingOptions {
  resolved_scope?: RequestedSessionScope;
}

export function isActionableCodexHookEvent(
  event: HookEvent,
  turnCaptureEnabled: boolean,
): boolean {
  if (event.event === "UserPromptSubmit" || event.event === "Stop") {
    return turnCaptureEnabled && Boolean(event.turn_id && event.captured);
  }
  if (event.event === "PreCompact") return Boolean(event.turn_id && event.trigger);
  return true;
}

function mappedScope(event: HookEvent, options: CodexEventMappingOptions): RequestedSessionScope {
  if (event.project_key) return event.project_key as RequestedSessionScope;
  if (event.event === "SessionStart" && event.scope_intent === "global") return "global";
  if (options.resolved_scope) return options.resolved_scope;
...
export function mapCodexHookEvent(
  ...
  const common = {
    ...
    occurred_at_source: event.occurred_at_source === "received" ? "received" as const : "host" as const,
    scope: mappedScope(event, options),
    identity: {
      key: event.event_id,
      source: event.event_id_source === "generated" ? "generated" as const : "host" as const,
    },
  };
