import { EvidenceReferenceSchema, type EvidenceReferenceInput } from "./provenance.js";

export interface EvidenceAdjustedConfidence {
  confidence: number;
  active: number;
  invalidated: number;
...
export function evidenceAdjustedConfidence(
  ...
  for (const item of parsed) counts[item.status] += 1;
  const total = parsed.length;
  if (total === 0) return { confidence: baseConfidence, ...counts };
