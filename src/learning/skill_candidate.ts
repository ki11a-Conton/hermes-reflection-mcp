...
export const SkillRevisionAuditSchema: z.ZodType<SkillRevisionAudit> = z.object({
...
export function skillRevisionContentHash(input: Omit<SkillRevision, "content_hash">): string {
...
export const SkillPromotionDirtyScopeSchema: z.ZodType<SkillPromotionDirtyScope> = z.object({
  scope: ScopeSchema,
...
export const SkillPromotionMetadataSchema: z.ZodType<SkillPromotionMetadata> = z.object({
...
export function isSkillPromotionScopeDirty(item: SkillPromotionDirtyScope): boolean {
  if (item.completed_at === undefined) return true;
...
const PROMOTION_TRANSITIONS: Readonly<Record<SkillPromotionState, readonly SkillPromotionState[]>> = {
...
function promotionFingerprintProjection(input: Pick<SkillPromotionCandidate,
  | "action"
  | "scope"
  | "target_skill_id"
...
export function skillPromotionCandidateFingerprint(input: Parameters<typeof promotionFingerprintProjection>[0]): string {
...
export const SkillPromotionCandidateSchema = SkillPromotionCandidateObjectSchema.superRefine((candidate, context) => {
  ...
  let state: SkillPromotionState = "pending";
...
}) as z.ZodType<SkillPromotionCandidate>;

export interface CreateSkillPromotionCandidateInput extends Omit<SkillPromotionCandidate,
  "state" | "fingerprint" | "updated_at" | "history"> {}
...
export function createSkillPromotionCandidate(
  input: CreateSkillPromotionCandidateInput,
): SkillPromotionCandidate {
...
export function transitionSkillPromotionCandidate(
  candidate: SkillPromotionCandidate,
  next: SkillPromotionState,
  at: string,
  reason: string,
): SkillPromotionCandidate {
