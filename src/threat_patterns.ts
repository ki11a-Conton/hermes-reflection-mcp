 ...
 */
export function scanForThreats(text: string, scope: ThreatScope): string[] {
  if (!text) return [];
 ...
 */
export function firstThreatMessage(text: string, scope: ThreatScope): string | null {
  ...
  return (
    `Content blocked: detected threat pattern(s): ${threats.join(", ")}. ` +
