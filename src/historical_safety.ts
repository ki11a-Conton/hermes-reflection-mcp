...
export interface HistoricalThreatMetadata {
  blocked_instruction_count: number;
  redacted_secret_count: number;
  affected_fields: string[];
  /** Optional v21 fields preserve the original metadata shape for older clients. */
  projection_truncated?: boolean;
  budget_exhausted?: boolean;
  reasons?: string[];
}

export interface SafeHistoricalTextResult {
  text: string;
  threat: HistoricalThreatMetadata;
  truncated: boolean;
}

export interface HistoricalRecordPolicy {
  mode?: "compact" | "full";
  defaultMaxChars?: number;
  fieldMaxChars?: Record<string, number>;
  includeThreatMetadata?: boolean;
  /** Aggregate source code points inspected across one projected record. */
  maxScanChars?: number;
  /** Defensive traversal bounds for manually edited or legacy persisted data. */
  maxDepth?: number;
  maxNodes?: number;
}
...
function corpusView(leaves: MutableStringLeaf[], separator: string): CorpusView {
  const parts: string[] = [];
  const starts: number[] = [];
  const ends: number[] = [];
  let cursor = 0;
...
function leafAt(view: CorpusView, position: number): number {
  let low = 0;
  let high = view.ends.length - 1;
  while (low < high) {
    const middle = Math.floor((low + high) / 2);
    if (position < view.ends[middle]) high = middle;
    else low = middle + 1;
  }
  return low;
}

function crossLeafSpan(view: CorpusView, span: Span): { first: number; last: number } | null {
  if (span.end <= span.start || view.ends.length < 2) return null;
  const first = leafAt(view, span.start);
  const last = leafAt(view, Math.max(span.start, span.end - 1));
  return first < last ? { first, last } : null;
}

function splitFenceSpans(view: CorpusView): Span[] {
  const spans: Span[] = [];
  ...
  while ((match = pattern.exec(view.text)) !== null) {
    ...
    if (match[1] !== "/") {
      if (stack.length < MAX_FENCE_DEPTH) stack.push({ kind, start: match.index });
      else {
        if (overflowDepth === 0) overflowStart = match.index;
        overflowDepth += 1;
      }
      continue;
    }
    ...
    if (overflowDepth > 0) {
      ...
      if (overflowDepth === 0) {
        ...
        if (crossLeafSpan(view, overflowEnvelope)) spans.push(overflowEnvelope);
      }
      continue;
    }
    let matchingDepth = -1;
...
function splitRoleDirectiveSpans(view: CorpusView): Span[] {
  ...
  while ((roleMatch = rolePattern.exec(view.text)) !== null) {
    ...
    for (const source of [PREVIOUS_INSTRUCTION_DIRECTIVE_RE, SECRET_DISCLOSURE_RE, DIRECTIVE_ACTION_RE]) {
      ...
      let threatMatch: RegExpExecArray | null;
    ...
    if (threatSpans.length === 0) continue;
...
function replaceSpans(value: string, spans: Span[]): { value: string; replacements: number } {
  const merged = mergeSpans(spans);
  if (merged.length === 0) return { value, replacements: 0 };
  let cursor = 0;
  const output: string[] = [];
...
function blockCrossFieldThreats(leaves: MutableStringLeaf[]): { blockedCount: number; affected: string[] } {
  const ranges = new Map<number, Span[]>();
  const mark = (index: number, span: Span) => {
  ...
  for (const separator of ["", " "]) {
    ...
    for (const pattern of [CROSS_FIELD_DIRECTIVE_RE, SECRET_DISCLOSURE_RE]) {
      ...
      while ((match = pattern.exec(view.text)) !== null) {
        ...
        if (crossLeafSpan(view, span)) spans.push(span);
        if (match[0].length === 0) pattern.lastIndex += 1;
      }
    }
    spans.push(...splitFenceSpans(view));
    if (separator === "") spans.push(...splitRoleDirectiveSpans(view));
    for (const span of spans) {
      if (span.end <= span.start || view.ends.length === 0) continue;
      const first = leafAt(view, span.start);
      const last = leafAt(view, Math.max(span.start, span.end - 1));
  ...
  for (const [index, spans] of ranges) {
    ...
    if (affected.length < 16) affected.push(leaf.path);
  }
  return { blockedCount, affected };
}

function blockRoleLineDirectives(value: string): string {
  return value.replace(ROLE_LINE_RE, (line) => {
    ...
    if (blockedContent === content) return line;
    return `${BLOCKED_HISTORICAL_INSTRUCTION} ${blockedContent.trimStart()}`;
  });
}

function blockHistoricalFences(value: string): string {
  ...
  while ((match = pattern.exec(scanned)) !== null) {
    ...
    const kind = match[2].toUpperCase();
    if (!closing) {
      if (stack.length === 0) output.push(scanned.slice(cursor, match.index));
      if (stack.length < MAX_FENCE_DEPTH) stack.push(kind);
      else overflowDepth += 1;
      continue;
    }
  ...
  if (stack.length > 0) {
    ...
    if (scanEnd < value.length) output.push(BLOCKED_HISTORICAL_INSTRUCTION);
  }
  return output.join("");
}
...
function needsStrictHistoricalUrlRedaction(value: string): boolean {
 ...
 */
export function safeHistoricalText(value: unknown, field: string, maxChars: number): SafeHistoricalTextResult {
...
export function safeHistoricalRecord<T>(record: T, policy: HistoricalRecordPolicy = {}): T {
  ...
  const fieldMaxByPath = new Map<string, number>();
  let blockedCount = 0;
  let redactedCount = 0;
  let projectionTruncated = false;

  const noteTruncation = (path: string, reason: string) => {
    projectionTruncated = true;
    if (truncationReasons.size < 8) truncationReasons.add(reason);
    if (affected.size < 16) affected.add(path || "$record");
  };

  const exhaustedValue = (value: unknown): unknown => {
    if (Array.isArray(value)) return [HISTORICAL_PROJECTION_TRUNCATED];
  ...
  const visit = (value: unknown, path: string, field: string, depth: number): unknown => {
    visitedNodes += 1;
    ...
    if (typeof value === "string") {
      ...
      if (remainingScanChars !== Number.MAX_SAFE_INTEGER) {
        ...
        for (const point of value) {
          if (points.length >= remainingScanChars) break;
          points.push(point);
        }
        source = points.join("");
    ...
    for (const [key, entry] of Object.entries(value as Record<string, unknown>)) {
      if (key === "historical_safety") continue;
  ...
  if (typeof projected === "string") {
    ...
    const collect = (value: unknown, path: string) => {
      ...
      if (Array.isArray(value)) {
        for (let index = 0; index < value.length; index += 1) {
          ...
          } else collect(entry, childPath);
        }
        return;
      }
      if (!value || typeof value !== "object") return;
      for (const [key, entry] of Object.entries(value as Record<string, unknown>)) {
        if (key === "historical_safety" || key === "historical_projection_truncated") continue;
        const childPath = path ? `${path}.${key}` : key;
        if (typeof entry === "string" && entry !== HISTORICAL_PROJECTION_TRUNCATED) {
          leaves.push({
            path: childPath,
            value: entry,
            set: (next) => { (value as Record<string, unknown>)[key] = next; },
          });
        } else collect(entry, childPath);
      }
    };
    ...
    for (const path of crossField.affected) if (affected.size < 16) affected.add(path);
    for (const leaf of leaves) {
      ...
      if (safe.threat.affected_fields.length > 0 && affected.size < 16) affected.add(leaf.path);
    }
  }
  ...
  if (includeMetadata && projected && typeof projected === "object" && !Array.isArray(projected)) {
    (projected as Record<string, unknown>).historical_safety = {
      blocked_instruction_count: blockedCount,
      redacted_secret_count: redactedCount,
    ...
    } satisfies HistoricalThreatMetadata;
  }
  return projected;
}
