...
export function semanticReviewRiskReasons(value: string): string[] {
  const risks: string[] = [];
  ...
  if (transientOrEnvironmental && permanentProhibition) {
    ...
    if (/\b(?:tool|command|api|provider)\b/i.test(value)) risks.push("unverified_tool_negation");
    if (/\b(?:once|one[- ]time|temporary|transient)\b/i.test(value)) risks.push("single_event_overgeneralization");
  }
