...
function nullableArray<T extends ZodTypeAny>(schema: T) {
...
const SessionMetadataTextSchema = z.string().min(1).max(100).refine(
...
export function defineTool<T extends RegisteredTool>(tool: T): T {
  if (tools.has(tool.name)) throw new Error(`Duplicate tool: ${tool.name}`);
  tools.set(tool.name, tool);
  return tool;
}

function profilesFor(name: string): ToolProfile[] {
  const profiles: ToolProfile[] = ["extended"];
  if (coreToolNames.has(name)) profiles.unshift("core");
  if (adminToolNames.has(name)) profiles.push("admin");
  return profiles;
}

function register(name: string, description: string, input: ZodTypeAny, annotations: ToolAnnotations) {
...
export function parseToolInput(name: string, value: unknown): unknown {
...
export function profileToolNames(profile: ToolProfile): string[] {
  if (profile === "core") return [...CORE_TOOL_NAMES];
  return [...tools.values()]
