...
import {
  buildCompactionHandoff,
  parseCompactionReceipt,
  type CompactionMetadata,
} from "./compaction_handoff.js";
 ...
 */
async function resolveManualReviewScope(sessionId: string, projectKey?: string) {
  try {
    return await resolvePersistedSessionAccess(sessionId, projectKey);
  } catch (error) {
    if (!(error instanceof SessionScopeError) || error.code !== "LIFECYCLE_NOT_READY") throw error;
