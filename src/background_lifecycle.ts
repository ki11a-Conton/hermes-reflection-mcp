...
import {
  ...
  mapCodexHookEvent,
} from "./adapters/codex/event_mapper.js";
import {
  dispatchLifecycleEvent,
  type LifecycleDispatcherPorts,
...
export class BackgroundLifecycle {
  ...
  private async applyHookEvent(event: HookEvent): Promise<void> {
    if (!isActionableCodexHookEvent(event, this.turnCaptureEnabled)) return;
    ...
    const ports: LifecycleDispatcherPorts = {
      ...
      stage_turn_side: async (canonical) => {
        if (!this.turnCaptureEnabled || !canonical.payload.capture) return;
      ...
      persist_compaction_observation: async (canonical, phase) => {
        if (!canonical.turn_id || !canonical.payload.observation) return;
      ...
      persist_compaction_receipt: async (canonical) => {
        if (!canonical.payload.trusted_receipt) return true;
        return Boolean(await persistCompactionReceipt(
          canonical.session_id,
          canonical.payload.trusted_receipt,
          canonical.scope,
        ));
      },
