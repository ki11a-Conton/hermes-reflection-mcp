...
function transferError(reason: string, nextStep: string): HermesError {
  return new HermesError("TRANSFER_PATH_DENIED", reason, false, nextStep);
}

function hasWindowsDeviceSyntax(value: string): boolean {
...
function configuredRoots(direction: TransferRequest["direction"]): string[] {
...
async function existing(value: string): Promise<boolean> {
  return stat(value).then(() => true, () => false);
}

async function canonicalCandidate(candidate: string, candidateExists: boolean): Promise<string> {
  if (candidateExists) return realpath(candidate);
  let parent = dirname(candidate);
  const suffix: string[] = [];
  while (!(await existing(parent))) {
    const next = dirname(parent);
    if (next === parent) throw transferError(
      "Transfer parent does not exist inside an allowed root.",
      "Create the parent directory under the configured transfer root first.",
    );
...
export async function resolveTransferTarget(request: TransferRequest): Promise<TransferTarget> {
  const defaultRoot = join(
    STORE_DIR,
    "transfers",
    request.direction === "export" ? "exports" : "imports",
  );
  ...
  if (!requested) throw transferError(
    "An import file name is required.",
    "Place a JSON file in transfers/imports and pass its relative file name.",
  );
  if (requested.includes("\0") || hasWindowsDeviceSyntax(requested)) {
    throw transferError(
      "Device paths and alternate data streams are not valid transfer targets.",
      "Use a normal .json file under an allowed transfer root.",
    );
  }
  if (request.direction === "import" && extname(requested).toLowerCase() !== ".json") {
    throw transferError(
      "Imports must use a .json file.",
      "Choose a JSON export under transfers/imports.",
    );
  }
  ...
  if (rootIndex < 0) {
    throw transferError(
      "Transfer path is outside an allowed root.",
      "Choose a file under the configured transfer directory.",
    );
  }
  ...
  if (request.direction === "import" && !candidateExists) {
    throw transferError(
      ...
      "Place the JSON file under transfers/imports before importing it.",
    );
  }
  if (request.direction === "export" && candidateExists && !request.overwrite) {
    throw transferError(
      "Target exists and overwrite was not confirmed.",
      "Pass overwrite:true or choose a new file name.",
    );
  }
  if (candidateExists) {
    const info = await stat(canonical);
    if (!info.isFile()) throw transferError(
      "Transfer target is not a regular file.",
...
export function redactExportValue(value: unknown): unknown {
  if (typeof value === "string") return redactSensitiveText(value, { strictHistorical: true });
...
export async function writeTransferJson(
