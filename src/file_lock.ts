...
async function quarantineStaleLock(lockPath: string, staleMs: number): Promise<boolean> {
  try {
    ...
    if (latest.mtimeMs > info.mtimeMs || Date.now() - latest.mtimeMs <= staleMs) return false;
    ...
    if (["ENOENT", "EACCES", "EPERM"].includes(errorCode(error) ?? "")) return false;
    throw error;
  }
}
  ...
  while (!handle) {
    try {
      ...
      if (Date.now() >= deadline) {
        ...
        throw new Error(
          `Timed out waiting for storage lock ${basename(lockPath)} after ${timeoutMs}ms (owner: ${owner})`,
        );
      }
      await delay(retryMs);
    }
