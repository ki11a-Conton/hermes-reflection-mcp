...
import { z } from "zod";
import { getLlmRuntimeReadiness, getLlmRuntimeSemanticFingerprint, runBoundedJsonTask, } from "./llm_transport.js";
...
function buildReviewTask(reflections) {
...
export async function runLlmReview(reflections, options = {}) {
    ...
    if (!transport.success || transport.output === undefined) {
        ...
        return failure(startedAt, transport.readiness, request.sourceIds, errorClass, message[errorClass], request.fingerprint);
    }
    try {
        ...
        return failure(startedAt, transport.readiness, request.sourceIds, "invalid_response", "LLM provider returned invalid or oversized structured output.", request.fingerprint);
    }
}
