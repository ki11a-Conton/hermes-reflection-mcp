...
function boundedNormalized(value) {
...
function textSimilarity(left, right) {
    ...
    if (normalizedLeft === normalizedRight)
        return 1;
    return Math.max(diceSimilarity(tokenSet(normalizedLeft), tokenSet(normalizedRight)), diceSimilarity(trigramSet(normalizedLeft), trigramSet(normalizedRight)));
}
function normalizedTagSet(tags) {
    return boundedSet(tags.map(normalizePromotionText).filter(Boolean));
}
function domainCompatibility(left, right) {
    const normalizedLeft = normalizePromotionText(left);
    const normalizedRight = normalizePromotionText(right);
    if (normalizedLeft === normalizedRight)
        return 1;
    if (normalizedLeft === "general" || normalizedRight === "general")
        return 0.5;
    return 0;
}
export function promotionProcedureGroundingScore(cluster, content) {
    const textScore = textSimilarity(cluster.normalized_text, [content.title, content.summary, ...content.steps].join("\n"));
...
function isProceduralReusable(heuristic) {
...
function hasUnsafePromotionSource(heuristic) {
...
function resolveEligibleHeuristic(heuristic, scope, reflectionMap) {
    if (heuristic.scope !== scope || heuristic.superseded_by !== undefined)
        return null;
    if (heuristic.contradiction_count > 0)
        return null;
    if (heuristic.feedback.some((item) => item.value === "harmful"))
        return null;
    if (!isProceduralReusable(heuristic) || semanticRiskReasons(heuristic).length > 0
        || hasUnsafePromotionSource(heuristic))
        return null;
    if (heuristic.evidence.length === 0)
        return null;
    const reflectionsById = new Map();
    for (const evidence of heuristic.evidence) {
        if (evidence.source_reflection_id === undefined)
            return null;
        const reflection = reflectionMap.get(evidence.source_reflection_id);
        if (reflection === undefined || reflection.scope !== scope)
            return null;
        if (hasUnresolvedFailure(reflection))
            return null;
        reflectionsById.set(reflection.id, reflection);
    }
    const reflections = [...reflectionsById.values()]
        .sort((left, right) => compareStableText(right.timestamp, left.timestamp) || compareStableText(left.id, right.id))
...
function heuristicsAreCompatible(left, right) {
    if (left.heuristic.scope !== right.heuristic.scope)
        return false;
    if (domainCompatibility(left.normalizedDomain, right.normalizedDomain) === 0)
        return false;
...
function repetitionSatisfied(reflections) {
    const successes = new Map();
    for (const reflection of reflections) {
        if (reflection.task_outcome === "success")
            successes.set(reflection.id, reflection);
    }
    if (successes.size < 2)
        return false;
    ...
    for (const reflection of successes.values()) {
        const goals = goalsBySession.get(reflection.session_id) ?? new Set();
        const normalizedGoal = normalizePromotionText(reflection.task_goal);
        if (normalizedGoal)
            goals.add(normalizedGoal);
        goalsBySession.set(reflection.session_id, goals);
    }
    return [...goalsBySession.values()].some((goals) => goals.size >= 3);
}
function representativeValue(values, fallback) {
    const counts = new Map();
    for (const value of values)
        counts.set(value, (counts.get(value) ?? 0) + 1);
    return [...counts.entries()]
        .sort(([leftValue, leftCount], [rightValue, rightCount]) => rightCount - leftCount || compareStableText(leftValue, rightValue))[0]?.[0] ?? fallback;
}
function projectCluster(members) {
    const sortedMembers = [...members].sort((left, right) => compareStableText(left.heuristic.id, right.heuristic.id) || left.heuristic.version - right.heuristic.version)
        .slice(0, MAX_CLUSTER_HEURISTICS);
    const reflections = [...new Map(sortedMembers.flatMap((member) => member.reflections)
            .map((reflection) => [reflection.id, reflection])).values()]
        .sort((left, right) => Number(right.task_outcome === "success") - Number(left.task_outcome === "success")
        || Date.parse(right.timestamp) - Date.parse(left.timestamp)
        || compareStableText(left.id, right.id))
        .slice(0, MAX_CLUSTER_REFLECTIONS);
    if (!repetitionSatisfied(reflections))
        return null;
    ...
    const normalizedText = takeCodePoints(sortedMembers.map((member) => member.normalizedText).join("\n"), 12_000);
    const clusterFingerprint = fingerprint({
        ...
        evidence_hashes: [...new Set(sortedMembers.flatMap((member) => member.heuristic.evidence.map((evidence) => evidence.content_hash)))].sort(compareStableText),
        domain,
        tags,
    });
    return {
...
export function buildPromotionClusters(snapshot) {
    ...
    const eligible = snapshot.heuristics
        ...
        .sort((left, right) => compareStableText(left.heuristic.scope, right.heuristic.scope)
    ...
    return groups.map(projectCluster).filter((cluster) => cluster !== null)
        .sort((left, right) => compareStableText(left.fingerprint, right.fingerprint));
}
export function matchPromotionTarget(cluster, skills) {
    const matches = skills.flatMap((skill) => {
        if (skill.scope !== cluster.scope || skill.status !== "active")
            return [];
