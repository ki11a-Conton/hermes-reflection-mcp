 ...
 */
async function dispatchDirectLifecycle(canonical, resolvedProjectKey, acceptedMetadata) {
    let snapshotResult;
    let releaseResult;
    ...
    const ports = {
        ...
        persist_compaction_receipt: async (event) => {
            if (!event.payload.trusted_receipt)
                return true;
            compactionReceipt = await persistCompactionReceipt(event.session_id, event.payload.trusted_receipt, event.scope);
...
export async function handleSessionLifecycleHook(args) {
    ...
    if (event === "start") {
        ...
        const { snapshotResult: captureResult } = await dispatchDirectLifecycle(canonical, resolvedProjectKey, acceptedMetadata);
        if (!captureResult)
            throw new Error("Lifecycle snapshot result is unavailable");
    ...
    if (event === "end") {
        ...
        if (!releaseResult)
            throw new Error("Lifecycle release result is unavailable");
    ...
    if (event === "postcompact") {
        ...
        const effects = await dispatchDirectLifecycle(canonical, resolvedProjectKey, acceptedMetadata);
        if (!effects.compactionReceipt)
            throw new Error(SESSION_STORAGE_UNAVAILABLE);
        if (!effects.snapshotResult)
            throw new Error("Lifecycle snapshot result is unavailable");
        if (!effects.snapshotResult.success)
            throw new Error(effects.snapshotResult.message);
        return JSON.stringify({
            success: true,
            event,
            session_id,
