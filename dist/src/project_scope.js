...
export function projectScope(key) {
    ...
    if (!PROJECT_KEY.test(key))
        throw new Error("project_key must contain 1-128 safe characters");
    return key.startsWith("project:") ? key : `project:${key}`;
}
function validateSessionId(value) {
    if (!SESSION_ID.test(value))
        throw new Error("session_id must contain 1-200 safe characters");
    return value;
}
...
function safeMetadata(value) {
    ...
    if (Object.keys(raw).some((key) => !allowed.has(key)))
        throw new Error("lifecycle metadata contains unsupported fields");
    const result = {};
...
function decodeScopeState(value) {
    if (!value || typeof value !== "object" || Array.isArray(value)) {
    ...
    if (entries.length > MAX_BINDINGS)
        throw new Error(`project scope bindings exceed ${MAX_BINDINGS}`);
...
export class FileProjectScopeRepository {
    path;
    constructor(path) {
        this.path = path;
    }
    async readUnlocked() {
        const loaded = await readAuthoritativeJson(this.path, "Hermes project scope state");
        if (!loaded.exists)
            return { schema_version: 1, bindings: Object.create(null) };
        try {
            ...
            throw new AuthoritativeStateError(`Refusing to continue: project scope state is invalid (${reason}). Evidence backup: ${backup}. Nothing was changed.`);
        }
    }
    ...
    async bind(sessionId, key, metadata) {
        validateSessionId(sessionId);
    ...
    async resolve(input) {
        if (input.project_key !== undefined)
            return projectScope(input.project_key);
        if (!input.session_id)
            return "global";
    ...
    async release(sessionId) {
        ...
        await withFileLock(this.path, async () => {
            const state = await this.readUnlocked();
            if (!Object.prototype.hasOwnProperty.call(state.bindings, sessionId))
                return;
...
export const projectScopeRepository = new FileProjectScopeRepository(join(homedir(), ".hermes-reflection", "project_scope.json"));
