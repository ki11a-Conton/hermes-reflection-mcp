...
const HOOK_FILE_LOCK_OPTIONS = { timeout_ms: 4_000, retry_ms: 25, stale_ms: 500 };
const SafeIdSchema = z.string().min(1).max(200).regex(/^[A-Za-z0-9._:-]+$/);
const CapturedTurnSchema = z.object({
    side: z.enum(["user", "assistant"]),
    content: z.string().refine((value) => codePointLength(value) <= MAX_CAPTURE_CODE_POINTS, `captured content exceeds ${MAX_CAPTURE_CODE_POINTS} Unicode code points`),
...
async function readOptional(path, maxBytes = MAX_QUEUE_BYTES) {
    try {
        const data = await readFile(path);
        if (data.byteLength > maxBytes)
            throw new Error("hook inbox state exceeds its hard byte limit");
...
function parseJsonl(raw, label) {
    ...
    if (lines.length > MAX_QUEUED_EVENTS)
        throw new Error(`${label} exceeds ${MAX_QUEUED_EVENTS} events`);
...
function eventIdentityHash(event) {
...
function completedMatches(event, completed) {
    ...
    if (completed.event_hash)
        return completed.event_hash === eventHash(event);
    return false;
}
function eventIdConflict(eventId) {
    return new Error(`HOOK_EVENT_ID_CONFLICT: event_id '${eventId}' is already bound to different canonical input`);
}
function completionForEvent(event, dedup) {
    ...
    if (!completedMatches(event, first))
        throw eventIdConflict(event.event_id);
    return first;
}
function completedIdsForPending(events, dedup) {
    ...
    for (const event of events) {
        ...
        if (completionForEvent(event, dedup))
            completed.add(event.event_id);
    }
    return completed;
}
function completionRecord(event, completedAt = new Date().toISOString()) {
    return {
        event_id: event.event_id,
        event: event.event,
        session_id: event.session_id,
...
function serializedDedup(state) {
...
function assertCompletionCapacity(dedup, event) {
...
export class HookInbox {
    ...
    stateLockPath;
    consumerLockPath;
    constructor(root = join(homedir(), ".hermes-reflection")) {
        this.root = root;
    ...
    async enqueue(input) {
        ...
        return withFileLock(this.stateLockPath, async () => {
            ...
            if (pendingMatch) {
                if (canonicalEventIdentity(pendingMatch) !== canonicalEventIdentity(event))
                    throw eventIdConflict(event.event_id);
                return { accepted: false, duplicate: true, event_id: event.event_id };
            }
            if (dedup.completed.length >= MAX_COMPLETED_EVENTS) {
    ...
    async hasPendingSessionStart(sessionId) {
    ...
    async consume(handler, observer = {}) {
        return withFileLock(this.consumerLockPath, async () => {
            const batch = await withFileLock(this.stateLockPath, async () => {
                ...
                if (processingRaw)
                    return parseJsonl(processingRaw, "hook processing inbox");
            ...
            for (const event of batch) {
                const alreadyCompleted = await withFileLock(this.stateLockPath, async () => {
                    ...
                    if (!completed)
                        assertCompletionCapacity(dedup, event);
