...
import { MAX_LIFECYCLE_CAPTURE_CODE_POINTS, } from "./events.js";
const SafeIdentifierSchema = z.string().min(1).max(200).regex(/^[A-Za-z0-9._:-]+$/);
const BoundedMetadataTextSchema = z.string().min(1).max(100).refine((value) => !value.includes("\r") && !value.includes("\n") && !value.includes("\0"), { message: "Must not contain CR, LF, or NUL" });
const HostSchema = z.object({
    name: z.string().min(1).max(100).regex(/^[A-Za-z0-9._:-]+$/),
    version: BoundedMetadataTextSchema.optional(),
}).strict();
const ScopeSchema = z.string().refine((value) => value === "global" || /^project:[A-Za-z0-9._:-]{1,128}$/.test(value), { message: "scope must be global or a non-empty project scope" });
...
const CaptureBaseShape = {
    content: z.string().refine((value) => codePointLength(value) <= MAX_LIFECYCLE_CAPTURE_CODE_POINTS, `captured content exceeds ${MAX_LIFECYCLE_CAPTURE_CODE_POINTS} Unicode code points`),
...
export function parseCanonicalLifecycleEvent(value) {
    return CanonicalLifecycleEventSchema.parse(value);
}
