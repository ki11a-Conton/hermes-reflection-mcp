...
const DEFAULT_EVENTS = ["SessionStart", "SessionEnd", "PreCompact", "PostCompact"];
const CAPTURE_EVENTS = ["UserPromptSubmit", "Stop"];
...
export function isHermesHookCommand(command, hookCliPath) {
...
function validateOptions(options) {
    if (!options || typeof options !== "object")
        throw new Error("install options must be an object");
    if (typeof options.node_path !== "string" || !options.node_path.trim())
        throw new Error("node_path must be a non-empty string");
    ...
    if (typeof options.capture_enabled !== "boolean")
        throw new Error("capture_enabled must be boolean");
...
function validateAndStripEvent(eventName, rawGroups, options) {
    ...
    for (const [groupIndex, rawGroup] of rawGroups.entries()) {
        ...
        if (!Array.isArray(group.hooks))
            throw new Error(`hooks.${eventName}[${groupIndex}].hooks must be an array`);
        const kept = [];
        for (const [hookIndex, rawHook] of group.hooks.entries()) {
            ...
            if (hook.type === "command") {
                ...
                if (isHermesHookCommand(hook.command, options.hook_cli_path) || isHermesHookCommand(hook.command))
                    continue;
            }
...
export function mergeCodexHooks(input, rawOptions) {
    ...
    for (const [eventName, groups] of Object.entries(hooks)) {
        ...
        if (stripped.length > 0)
            mergedHooks[eventName] = stripped;
    }
    ...
    for (const eventName of enabled) {
        const groups = Array.isArray(mergedHooks[eventName]) ? mergedHooks[eventName] : [];
        groups.push({ hooks: [hermesHook(options)] });
        mergedHooks[eventName] = groups;
    }
    root.hooks = mergedHooks;
    return JSON.parse(canonicalCodexHooksJson(root));
}
export function canonicalCodexHooksJson(value) {
