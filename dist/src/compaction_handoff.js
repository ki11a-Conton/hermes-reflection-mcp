...
export function createCompactionReceipt(input) {
...
export function parseCompactionReceipt(value) {
    ...
    const receipt = { ...metadata, status, receipt_hash: receiptHash };
    const expected = createCompactionReceipt(metadata);
    if (receipt.receipt_hash !== expected.receipt_hash)
        throw new Error("COMPACTION_RECEIPT_INVALID: receipt hash mismatch");
    return receipt;
}
...
function truncateUtf16Safe(value, maxUnits) {
    if (maxUnits <= 0)
        return "";
    let used = 0;
    let output = "";
    for (const character of value) {
        if (used + character.length > maxUnits)
            break;
        output += character;
...
export function isContextHandoffContent(value) {
...
function lineWithBudget(label, value, valueBudget, emptyText) {
    if (!value)
        return { line: emptyText, truncated: false };
...
export function buildCompactionHandoff(turns, reflections, maxTurns, maxChars, preserveRecentUserTurns = 3) {
    ...
    const lastAssistant = [...ordinaryTurns].reverse().find((turn) => turn.role === "assistant" && turn.content.trim().length > 0);
    ...
    const userAnchor = lineWithBudget(userLabel, lastUser?.content, userBudget, "No stored user turn.");
    const lines = [
        PREFIX,
        "",
        "## Historical Task Snapshot",
    ...
    const includedEarlierRecentUsers = [];
    const lengthWithFooter = (candidateLines) => [...candidateLines, "", CONTEXT_HANDOFF_END_MARKER].join("\n").length;
    if (lastAssistant) {
        ...
        if (assistantBudget > 0) {
            const assistantAnchor = lineWithBudget(assistantLabel, lastAssistant.content, assistantBudget, assistantLabel);
            lines.push("", title, assistantAnchor.line);
            if (assistantAnchor.truncated)
    ...
    for (const turn of [...earlierRecentUsers].reverse()) {
        ...
        const proposedLines = proposed.map((item) => `- [turn ${item.turn_index}] ${oneLine(item.content)}`);
    ...
    for (const section of optionalSections) {
        ...
        for (const item of sectionLines) {
            ...
            if (lengthWithFooter([...lines, rendered]) > maxChars)
                break;
            lines.push(rendered);
            includedInSection += 1;
            if (section.countAsReflectionItems && section.lines.length > 0)
                reflectionItemsIncluded += 1;
        }
        if (includedInSection < sectionLines.length) {
            sectionsTruncated.push(section.title.slice(3));
            const marker = "- ...[section truncated]";
            if (lengthWithFooter([...lines, marker]) <= maxChars)
                lines.push(marker);
        }
    }
    ...
    return {
        ...
        source: {
            ...
            recent_user_turns_omitted_due_to_budget: recentUserTurns.length - ((lastUser ? 1 : 0) + includedEarlierRecentUsers.length),
