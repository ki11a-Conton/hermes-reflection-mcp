...
import { STORE_DIR, exportData, previewClearData, previewReplaceImportData, replaceStoreDataSnapshot, withOperationJournalStoreMutation, withStoreSnapshotBarrier, } from "../storage.js";
import { emptySessionStorageSnapshot, replaceSessionStorageSnapshot, snapshotSessionStorage, snapshotSessionStorageAfterWriteBarrier, validateSessionStorageSnapshot, withOperationJournalSessionMutation, } from "../session_storage.js";
...
const PHASES = new Set(["prepared", "json_staged", "sqlite_staged", "committing", "complete"]);
const NEXT_PHASE = {
...
function validateRelativePath(path, id) {
    ...
    if (!path.startsWith(prefix))
        throw new Error("operation journal path is outside its operation directory");
...
export function decodeOperationJournal(value) {
    ...
    if (typeof input.created_at !== "string")
        throw new Error("operation journal created_at is invalid");
    ...
    const staged = input.staged_paths;
    const backups = input.backup_paths;
    ...
    for (const path of [...staged, ...backups])
        validateRelativePath(path, input.id);
    return {
        schema_version: 1,
        id: input.id,
...
export function hashJournalJsonState(store) {
...
async function ensureDirectoryIsSafe(path, label) {
    const info = await lstat(path);
    if (!info.isDirectory() || info.isSymbolicLink())
        throw new Error(`${label} must be a real directory, not a link`);
}
async function prepareDirectories(id) {
...
async function transition(record, next) {
...
async function markRecoveredComplete(record) {
    const current = await readJournal();
    if (!current || current.id !== record.id || current.phase !== record.phase) {
    ...
    if (current.phase === "complete")
        return current;
    const updated = { ...current, phase: "complete" };
    await writeJournal(updated);
    return updated;
}
...
async function cleanup(record) {
    ...
    if (relative(resolve(OPERATIONS_DIR), directory).startsWith(".."))
        throw new Error("unsafe operation cleanup directory");
    await rmdir(directory).catch((error) => {
        if (!(error && typeof error === "object" && "code" in error && error.code === "ENOENT"))
            throw error;
    });
}
async function ensureRecordDirectoriesSafe(record) {
    await ensureDirectoryIsSafe(OPERATIONS_DIR, "operations directory");
...
async function prepare(operation, snapshots) {
    if (existsSync(JOURNAL_PATH))
        throw new Error("another recoverable cross-store operation is pending");
    const id = randomUUID();
...
function maybeFail(failpoint) {
    if (process.env.HERMES_TEST_OPERATION_FAILPOINT === failpoint)
        throw new InjectedOperationFailure(failpoint);
}
async function stageJson(record, snapshot) {
    if (hashJournalJsonState(snapshot) !== record.after.json)
        throw new Error("staged JSON hash does not match journal");
    await atomicWriteJson(validateRelativePath(record.staged_paths[0], record.id), snapshot);
    return transition(record, "json_staged");
}
async function stageSqlite(record, snapshot) {
    if (hashJournalSqliteState(snapshot) !== record.after.sqlite)
        throw new Error("staged SQLite hash does not match journal");
...
async function loadJsonArtifact(record, kind) {
    const index = kind === "before" ? 0 : 0;
    const relativePath = kind === "before" ? record.backup_paths[index] : record.staged_paths[index];
    const value = await readArtifact(validateRelativePath(relativePath, record.id));
    if (hashJournalJsonState(value) !== record[kind].json)
        throw new Error(`${kind} JSON recovery artifact hash mismatch`);
    return value;
}
async function loadSqliteArtifact(record, kind) {
    ...
    if (hashJournalSqliteState(value) !== record[kind].sqlite)
        throw new Error(`${kind} SQLite recovery artifact hash mismatch`);
    return value;
}
async function commitJson(snapshot) {
    await withOperationJournalStoreMutation(() => replaceStoreDataSnapshot(snapshot));
}
async function commitSqlite(snapshot) {
    const replaced = await withOperationJournalSessionMutation(() => replaceSessionStorageSnapshot(snapshot));
    if (!replaced)
        throw new Error("SQLite session storage is unavailable during recoverable operation");
}
async function verifyHashes(record, kind) {
    const liveJson = await exportData();
    const liveSqlite = await snapshotSessionStorage();
    if (!liveSqlite)
        throw new Error("SQLite session storage is unavailable during operation verification");
    if (hashJournalJsonState(liveJson) !== record[kind].json)
        throw new Error(`${kind} JSON verification hash mismatch`);
    if (hashJournalSqliteState(liveSqlite) !== record[kind].sqlite)
        throw new Error(`${kind} SQLite verification hash mismatch`);
}
...
async function acquireLock() {
    ...
    for (let attempt = 0; attempt < 400; attempt += 1) {
        try {
            ...
            let staleToken;
            try {
                const owner = JSON.parse(await readFile(LOCK_PATH, "utf8"));
...
async function execute(operation, prepareJson) {
    return withLock(async () => {
        ...
        if (!beforeSqlite)
            throw new Error("SQLite session storage is unavailable; no operation journal was created");
        ...
        try {
            ...
            if (error instanceof InjectedOperationFailure)
                throw error;
            try {
                await recoverUnlocked();
            }
