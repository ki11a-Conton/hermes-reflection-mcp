...
import { commitSkillPromotionFingerprint, dirtySkillPromotionScopes, enqueueSkillCandidateRecords, getSkillPromotionSnapshot, MAX_SKILL_CANDIDATES_PER_BATCH, SKILL_CANDIDATE_CAPACITY_CODE, skillCandidateIdsExist, skillPromotionEvidenceFingerprint, } from "../storage.js";
import { buildPromotionClusters, matchPromotionTarget, SKILL_CLUSTER_ALGORITHM, } from "./learning/skill_promotion.js";
import { getSkillSynthesisSemanticFingerprint, synthesizeSkillRevision, } from "./skill_synthesis.js";
...
function currentSkillProjection(skill) {
...
function promotionSourceFingerprint(snapshot, clusters, semanticFingerprint) {
    return fingerprint({
...
function result(scope, sourceFingerprint, outcomeClass, success, candidateIds = [], generated = 0, skipped = 0) {
    return {
        success,
        scope,
...
async function runPromotion(options, dependencies) {
    ...
    if (isAborted(options.signal))
        return result(options.scope, emptyFingerprint, "aborted", false);
    try {
        ...
        const sourceFingerprint = promotionSourceFingerprint(snapshot, clusters, dependencies.semantic_fingerprint());
        latestSourceFingerprint = sourceFingerprint;
        if (isAborted(options.signal))
            return result(options.scope, sourceFingerprint, "aborted", false);
        if (dirty.completed_fingerprint === sourceFingerprint) {
            const reconcile = async () => {
                ...
                if (isAborted(options.signal))
                    return "aborted";
                return await dependencies.commit(options.scope, dirty.dirty_at, sourceFingerprint, "unchanged", []) ? "unchanged" : "dirty_state_changed";
            };
            const reconciled = options.with_apply_lease
                ? await options.with_apply_lease(reconcile)
                : await reconcile();
            const outcome = reconciled ?? "lease_lost";
            return result(options.scope, sourceFingerprint, outcome, outcome === "unchanged");
        }
        const drafts = [];
        ...
        for (const cluster of clusters) {
            ...
            drafts.push({
                ...
                evidence_fingerprint: dependencies.evidence_fingerprint(snapshot, cluster.heuristic_ids, cluster.reflection_ids),
        ...
        const persist = async () => {
            ...
            const persistedOutcome = candidates.some((candidate) => candidate.state === "rejected")
                ? "blocked_candidates"
                : desiredOutcome;
            const committed = await dependencies.commit(options.scope, dirty.dirty_at, sourceFingerprint, persistedOutcome, candidateIds);
            return committed
        ...
        if (persisted === undefined)
            return result(options.scope, sourceFingerprint, "lease_lost", false);
        ...
        return result(options.scope, sourceFingerprint, persisted.outcomeClass, true, candidateIds, candidateIds.length, Math.max(0, clusters.length - candidateIds.length));
    }
    catch (error) {
        const code = error && typeof error === "object" && "code" in error
            ? String(error.code)
            : undefined;
        return result(options.scope, latestSourceFingerprint, isAborted(options.signal)
            ? "aborted"
...
function dependencyId(dependencies) {
...
export function runSkillPromotionSingleFlight(options, dependencies = defaultDependencies) {
